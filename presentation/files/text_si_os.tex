...
\begin{frame}{Door and Robot Orientations}
	\begin{columns}
		...
		\begin{itemize}[<+->]
			\item \textbf{Goal:} \textbf{estimate} how the \textbf{door signs are organized} in the environment
...
\begin{frame}{Distance}
	\begin{columns}[T]
		...
		\begin{itemize}[<+->]
			...
			\item It guides the robot towards the \textbf{closest frontier}, $\varphi_d({c}) = 1$
...
\begin{frame}{Final formula}
	\begin{columns}[T]
		...
		\begin{itemize}
			...
			\begin{itemize}
				\item \textbf{Semantic} part:\\$\mathtt{S}({c}) = \varphi_g({c}) \varphi_p({c})$			
			\end{itemize}
			\begin{itemize}
				\item \textbf{Geometric} part:\\$\mathtt{G}({c}) = \frac{\big(\varphi_o({c}) + \varphi_r({c})\big)\varphi_d({c}) + \varphi_d({c})}{3.0}$
			\end{itemize}	
%I SHOULD UNCOMMENT THIS ########################################################################################				
%			\item Find the \textbf{most promising frontier}: $\varphi({c}) &= \mathtt{S}({c}) * \alpha + \mathtt{G}({c}) * (1.0 - \alpha)$	\\ %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~${c}_* = \underset{{c} \in \mathbf{C}}{\textup{\textup{arg max}}}(\varphi({c}))$			
%I SHOULD UNCOMMENT THIS ########################################################################################
		\end{itemize}
...
\begin{frame}{Summary}
	\begin{columns}[T]
		...
		\begin{itemize}[<+->]
			...
			\end{itemize}
			\item It is \textbf{possible} to \textbf{expand} the \textbf{robot's perception} with semantic information
