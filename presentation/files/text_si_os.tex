	...
	\begin{columns}[T]
		...
		\column{.45\textwidth}
			...
			\begin{figure}
				 ...
				 \caption{\tiny Signs on primary routes, green backgrounds​.\footnotemark[11]}			 		 
	         \end{figure}			
	\end{columns}
	\footnotetext[11]{\tiny Figure extracted from \url{highwaycodeuk.co.uk/direction-signs-on-roads-motorways.html}.}						
\end{frame}
%========================================== ############# ======================================================
%========================================== ############# ======================================================
%========================================== ############# ====================================================== 
...
\begin{frame}[noframenumbering]{Map Segmentation module}
	\begin{columns}[T]
		...
		\begin{itemize}
			\item \textbf{Divides} the map based on the \textbf{free space}	
			\item \textbf{Simultaneously} to the \textbf{Mapping module}
			\item Kernel Density Estimation (\textbf{KDE})\footnotemark[15]
		\end{itemize}
		\column{.5\textwidth}	
			\centering	
			\begin{figure}
	         ...
	         \end{figure}		         		
	\end{columns}		
	\footnotetext[15]{\tiny Maffei, Renan, et al. ``\textit{Fast monte carlo localization using spatial density information.}'', ICRA, 2015}.							
\end{frame}
%========================================== ############# ======================================================
%========================================== ############# ======================================================
%========================================== ############# ======================================================
...
\begin{frame}[noframenumbering]{Image Processing module}
	\begin{columns}[T]
		...
		\begin{itemize}
			\item Optical character recognition (\textbf{OCR})
			\item \textbf{Recognize} the number of \textbf{door signs} in \textbf{RGB images}
			\item We use the work proposed by Neumann and Matas (2012)\footnotemark[16]
		\end{itemize}
		\column{0.5\textwidth}
			\centering
			\begin{figure}
	        ...
	        \end{figure}			
	\end{columns}	
	\footnotetext[16]{\tiny Neumann, Lukáš, and Jiří Matas. ``\textit{Real-time scene text localization and recognition}.'',Conference on Computer Vision and Pattern Recognition. 2012.}	
\end{frame}
%========================================== ############# ======================================================
%========================================== ############# ======================================================
%========================================== ############# ======================================================
			...
			\begin{figure}
			    \begin{subfigure}{.6\columnwidth}
					 \includegraphics[width=1\linewidth]{figs/parity_even.png}
     				 \caption{\tiny Door signs and goal-door with same parity.}			 		 
			    	\end{subfigure}\\
			    \begin{subfigure}{.6\columnwidth}
			    		\includegraphics[width=1\linewidth]{figs/parity_odd.png}	         
			    		\caption{\tiny Door signs and goal-door with different parity.}					 
			    	\end{subfigure}					 
...
\begin{frame}{Growing Direction}
	\begin{columns}[T]
		\column{0.5\textwidth}
			\centering
			\begin{itemize}
				\item \textbf{Goal}: estimate whether the segment is promising regarding the sequence of door signs
...
\begin{frame}{Growing Direction}
	\begin{columns}[T]
		...
		\column{1\textwidth}
		\begin{itemize}
			\item Compute the angle in which the door sign sequence is increasing
...
\begin{frame}{Growing Direction}
	\begin{columns}[T]
		...
		\begin{itemize}
			...
			\item Comparison with the goal-door (smaller and larger numbers are counted)
		\end{itemize}
	\end{columns}	
	\begin{columns}
		\centering
...
\begin{frame}{Door and Robot Orientations}
	\begin{columns}
		...
		\begin{itemize}[<+->]
			\item \textbf{Goal:} estimate how the door signs are organized in the environment
...
\begin{frame}[noframenumbering]{Door and Robot Orientations}
	\begin{columns}
		\column{0.55\textwidth}
		\begin{itemize}[<+->]
			\item Histograms are computed (each bin corresponds to an angle)
			\begin{itemize}
...
\begin{frame}{Distance}
	\begin{columns}[T]
		...
		\begin{itemize}[<+->]
			...
			\item It guides the robot towards the closest frontier, $\varphi_d({c}) = 1$
...
\begin{frame}{Final formula}
	\begin{columns}[T]
		...
		\begin{itemize}[<+->]
			...
			\begin{itemize}
				\item Semantic part:\\$\mathtt{S}({c}) = \varphi_g({c}) \varphi_p({c})$			
			\end{itemize}
			\begin{itemize}
				\item Geometric part:\\$\mathtt{G}({c}) = \frac{\big(\varphi_o({c}) + \varphi_r({c})\big)\varphi_d({c}) + \varphi_d({c})}{3.0}$
			\end{itemize}		
			\item Find the most promising frontier: $\varphi({c}) &= \mathtt{S}({c}) * \alpha + \mathtt{G}({c}) * (1.0 - \alpha)$	\\ ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~${c}_* = \underset{{c} \in \mathbf{C}}{\textup{\textrm{}arg max}}(\varphi({c}))$
%\begin{align}
...
\begin{frame}{Summary}
	\begin{columns}[T]
		\column{1\textwidth}
		\begin{itemize}[<+->]
			\item \textbf{Semantic OS system} that:
			\begin{itemize}
				\item  Relies on \textbf{semantic information inferred from text} within the environment
