\documentclass[aspectratio=169,11pt]{beamer}

%%%%%%%% tema e cor %%%%%%%%
\mode<presentation> {
\usetheme{CambridgeUS}
\usecolortheme{rose}
\usefonttheme{structuresmallcapsserif}

\beamertemplatenavigationsymbolsempty

\setbeamercolor{itemize item}{fg=red}
\setbeamercolor{itemize subitem}{fg=red}
\setbeamercolor{itemize subsubitem}{fg=red}

\setbeamertemplate{itemize item}[circle]
\setbeamertemplate{itemize subitem}[square]
\setbeamertemplate{itemize subsubitem}[triangle]

\setbeamerfont{caption}{size=\tiny}
\setbeamercolor{caption name}{fg=black}
}

\input{config.tex}

%%%%%%%% titulo e subtitulo %%%%%%%%
\title[Ph.D. Thesis]{Exploiting organisational semantic information in indoor environments for the object search problem} 

%%%%%%%% nome dos autores %%%%%%%%
\author[Mantelli, M. F.]{Mathias Fassini Mantelli} 


\begin{document}
%\setbeamertemplate{caption}{\raggedright\insertcaption\par}
\input{files/pre-slides.tex}

%%%%%%%% slides %%%%%%%%
\section{Introduction} 
\input{files/introducao.tex}

\section{Theoretical Background}
\input{files/background.tex}

\section{Text and OS}
\input{files/text_si_os.tex}
%\section{Objetivos}
%\input{files/objetivos.tex}

\section{Semi dynamic objects and OS}
\input{files/temporal_si_os.tex}
%\section{Fundamentação Teórica}
%\input{files/fundamentacao.tex}

\section{Application of the proposed OS systems}
\input{files/thesis_application.tex}

\section{Conclusion and future work}
\input{files/thesis_progress.tex}
%=========================================================================
%\section{Metodologia}
%\input{files/metodologia.tex}

%\section{Resultados}
%\input{files/resultados.tex}
%
%\section{Conclusão}
%\input{files/conclusao.tex}

%%%%%%%% agradecimentos %%%%%%%%
%\begin{frame}{Agradecimentos}
%    \large{Agradeço a fulano, ciclano e beltrano que apoiaram o desenvolvimento dessa pesquisa.}
%\end{frame}
%------------------------------------------------


%------------------------------------------------
%------------------------------------------------
%%%%%%%% referencias %%%%%%%%
%\nocite{*}
%\begin{frame}[allowframebreaks]{Referências}
%\bibliographystyle{unsrt}
%\bibliography{ref.bib}
%\end{frame}


%%%%%%%% repete primeiro slide %%%%%%%%
\begin{frame}
\titlepage 
\end{frame}

\section{APPENDIX}
\input{files/appendix.tex}


\end{document}