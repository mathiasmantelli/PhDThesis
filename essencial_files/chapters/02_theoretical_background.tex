...
There are two methodologies commonly used in OCR systems, integrated and stepwise~\cite{Ye2014Text}. The Integrated methodology recognizes words when the detection procedures share information with character classification and relies on joint optimization strategies, as shown in Fig.~\ref{chp02_fig:integrated_methodlogy}. On the other hand, Stepwise methodologies have separated detection and recognition modules. Besides, it uses a feed-forward pipeline to detect, segment, and recognize text regions, also shown in Fig.~\ref{chp02_fig:stepwise_methodology}~\cite{Ye2014Text}. Lastly, it relies on a feedback procedure from text recognition to text detection to reduce false detections.
...
\begin{figure}[ht!]
    ...
    \end{subfigure}\\[0.1cm]
    \caption[OCR PAPER.]{OCR PAPER~\cite{Neumann2012Realtime}.}
...
%    \footnotesize
    ...
    \end{subfigure}
    \caption[DEEP LEARNING SURVEY.]{DEEP LEARNING SURVEY~\cite{Liu2020Deep}.}
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption[YOLO SYSTEM MODEL.]{YOLO SYSTEM MODEL. Adapted from~\cite{Redmon2016You}.}
...
The process of computing the probability density function $f$ of any data sample $X = \{x_1, x_2,x_3, \cdots, x_n\}$ is called density estimation~\cite{Devroye1999Hilbert, Maffei2017Translating, Bhattacharjee2021KAGO}. The kernel density estimatation (KDE), $\hat{f}_h(x)$, is a non-parametric estimate of $f$ computed at point $x$, defined as
\begin{equation}
\hat{f}_h(x) = \frac{1}{n}\sum^n_{i=1}K_h(x - x_i)
\label{chp02_eq:kde}
\end{equation}
where $K_h(d)$ is a circular kernel function that operates over all points at distance $d \leq h$ from $x$, and $h$ is called the bandwidth of the kernel~\cite{Silverman1986Density, Jones1992Class}. 

For any given point $x_i \in X$ where $1 \leq i \leq n$, the KDE on dataset $X$ is used to estimate the likelihood of a point $x_i$ being drawn from $X$. The probability estimated through the kernel density estimator may be interpreted as the “point density” at any $x_i \in X$~\cite{Devroye1999Hilbert, Bhattacharjee2021KAGO}.

A variety of kernel profiles may be applied for estimating the density using KDE. Even thought the Gaussian kernel is one of the most frequently used kernel functions, there are also other kernel profiles that are used by the research community~\cite{Silverman1986Density, Maffei2015Fast}. Below we show the uniform, gaussian, and the inverted ones: 
\begin{equation}
...
%        \caption{}
    ...
    \end{subfigure}  
    \caption[KERNELS.]{KERNELS~\cite{Maffei2017Translating}.}
    \label{chp02_fig:kernel_profiles}
\end{figure} 

The KDE is a fundamental too in statistics~\cite{Devroye1985Nonparametric, Silverman1986Density, Devroye2001Combinatorial, Scott2015Multivariate} and machine learning~\cite{Scholkopf2002Learning, Gretton2012Kernel, Muandet2017Kernel}. However, it can also be use in many other fields, such as robotics, like in the work proposed by~\citet{Maffei2015Fast}. In their work, the authors proposed an efficient KDE-based observation model for a localization approach. The KDE is estimated considering the density of free space cells in a 2D grid map, since these cells are more abundant and less noisy than the obstacle cells. One big advantage of using KDE for robotic applications is that it is orientation independent and has a low cost in terms of computational costs~\cite{Maffei2015Fast}. Their KDE of free space surrounding a point $x$ is computed through an equation similar to Equation~\ref{chp02_eq:kde}:
\begin{equation}
\hat{f}_h(x) = \frac{1}{n}\sum^n_{i=1}s(x)K_h(x - x_i)
\label{chp02_eq:free_space_kde}
\end{equation}
where the function $s(\cdot)$ is defined as

\begin{equation}
s(x) = \left\{\begin{matrix}1 \text{, if}~x~\text{is inside a free space cell}\\ 
              \hspace{-3.2cm} 0 \text{, otherwise.}\end{matrix}\right.
\end{equation}

Figure~\ref{chp02_fig:kde_uniform} shows the Equation~\ref{chp02_eq:free_space_kde} with an uniform kernel applied to a 2D grid map. Given four map cells in four different spots of the map (1, 2, 3, and 4) indicated in Figure~\ref{chp02_fig:kde_points}, the circular KDE would consider the neighbor cells highlighted in Figure~\ref{chp02_eq:free_space_kde}. The function $s(\cdot)$ in Equation~\ref{chp02_eq:free_space_kde} makes the KDE to consider only the free cells, as shown in Figure~\ref{chp02_fig:kde_free_cells}. The free space density estimation for each cell, estimated with KDE, is illustrated in Figure~\ref{chp02_fig:kde_info}. In this image, it is possible to see that the larger the amount of free cells within the kernel area, the higher is the density. Besides, this simple example also demonstrates the fact that KDE is invariant to rations, as the KDE for points 1 and 4 are equal, even though the free space are in different orientations. The point 3 has the highest KDE, which is explained by the largest area of free cells under the kernel coverage. 
...
The idea of estimating the free space density with the aid of KDE was later used by Maffei and colleagues in a second workd~\citet{Maffei2015Using}. This time, they were interested in using a robot to construct a topological map to localize itself using a laser range finder and odometry information. Their proposed algorithm uses an observation model based on kernel density estimates, similarly to their previous work, ~\citet{Maffei2015Fast}. The difference, however, is that this observation model separes the map into regions denominated workds, classified based on the density of free space estimated with the KDE, number of observations and segmentation orientation~\cite{Maffei2015Using}. The robustness of the free space density estimates in this second work motivated the authors to pursue a third work,~\citet{Maffei2016Long}. Now,~\citet{Maffei2016Long} proposed a long-term place recognition, which extends their strategy that represents environment regions using words, based on spatial density information extracted from laser readings. The novelty is in building multi-level words to account for possible changes in the observations of a place generaged by non-static objects. For example, when a robot is in front a door and estimates the free space density, the final KDE value will be different whether the door is open or not. For long-term opperations, this may happen, and even thought the density value is different, the place is still the same. 
...
\begin{figure}[ht!]
    ...
    \end{subfigure}
    \caption[Frontier-based exploration.]{Frontier-based exploration.}
    \label{chp02_fig:frontier_exploration}
\end{figure} 

Among the frontier-guided exploration approaches for exploratory tasks we highlight the squence of works proposed by Prestes and colleagues~\cite{Prestes2002Exploration, Prestes2003BVPExploration, Prestes2011Exploration}. Initially,~\citet{Prestes2002Exploration} proposed the use of a numeric solution of a boundary value problem (BVP) for the exploration problem. Their BVP exploration approach uses the gradient descent of the potential field generated over a grid to perform the map coverage with the robot. Using the finite difference method, this potential field is computed by solving the Laplace equation
\begin{equation}
\nabla^2 p(\mathbf{r}) = \sum^n_{i=1}\frac{\partial^2p(\mathbf{r})}{\partial x^2_i} = 0,
\label{chp02_eq:laplace_equation}
\end{equation} 
where $p(\mathbf{r})$ is the potential value at position $\mathbf{r}$ in the free space. In the grid map, both the boundary of obstacles and unknown regions are considered as Dirichlet boundary conditions. The potential value associated to unknown cells is the minimum, i.e. 0, whereas the same value associated to obstacle cells is the maximum, i.e. 1. The free cells have their potential value updated with Equation~\ref{chp02_eq:laplace_equation}~\cite{Prestes2002Exploration, Prestes2003BVPExploration}. However, as we are working with a discrete regular grid map, we use the Gauss-Seidel method for the numerical approximation of Equation~\ref{chp02_eq:laplace_equation}. Therefore, to update the potential value of free cells we do
\begin{equation}
p(\mathbf{r}) = \frac{1}{4}(p(\mathbf{r_l}) + p(\mathbf{r_r}) + p(\mathbf{r_u}) +p(\mathbf{r_b}))
\label{chp02_eq:gauss_seidel}
\end{equation}
where $p(\mathbf{r_l})$, $p(\mathbf{r_r})$, $p(\mathbf{r_u})$, and $p(\mathbf{r_b})$ are the left, right, top and bottom cells neighboring the center and reference cell $p(\mathbf{r})$. Figure~\ref{chp02_fig:grid_potential} illustrates an example of these cells. 
\begin{figure}[ht!]
    \footnotesize
    \centering
...
The Equation~\ref{chp02_eq:gauss_seidel} applied to a 2D grid map to update the potential results in the exploratory behavior illustrated by Figure~\ref{chp02_fig:bvp_exploration}. The BVP exploration approach uses the gradient descent of the potential field over a grid map to perform the map coverage.  
The angle of the robot's heading is computed based on the robot's current position on the potential field, i.e.
\begin{equation}
\theta = \arctan(p(\mathbf{r_l} - p(\mathbf{r_r}, p(\mathbf{r_u} - p(\mathbf{r_b})
\end{equation}
where $\arctan(x,y)$ is the inverse tangent taken in the interval $[-\pi, \pi]$. The robot's speed can also be adjusted based on the potential field. The difference between the robot's orientation and the negative potential gradient direction $\theta$ for the current position, represents how much the robot has to turn. Hence, this difference can be correlated to the robot's speed, i.e., the larger the difference, the slower is the robot's speed~\cite{Prestes2003BVPExploration}. Therefore, the BVP exploration has many advantages, including but not limited to smooth movements to the robot, easy understanding and implementation, and there is no local minima in the resulting potentiel field~\cite{Maffei2014Integrated, Jorge2015Ouroboros}.
...
\medskip
    ...
    \end{subfigure}    
    \caption[BVP Exploration.]{BVP Exploration by~\cite{Prestes2002Exploration}.}
