...
A popular deep learning approach for object detection that has been updated since its first version is called YOLO~\cite{Redmon2016You}. The name is an acronym, and is short for ``\textit{You Only Look Once}'', which partially explains the general idea o the approach. The YOLO's authors have completely abbandoned the initial object detection paradigm of ``\textit{proposal detection + verification}''. Instead, they followed a quite different idea: to apply a single neural network to the full image. This idea made YOLO the first one-stage object detector in deep learning era (where the name comes from)~\cite{Zou2019Object}.
