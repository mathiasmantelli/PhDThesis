...
Exploration is one of the robotic problems that the research community has been working on for many decades~\cite{Thrun2006Probabilistic}. Approaches that deal with this problem comes in handy when a robot is deployed in an environment which map is not available. This scenario requires the robot to safely move around within the environment at same time that it builds the map. This problem has two fundamental requirements: the safety of the robot while it is moving, and the preference to movements towards unexplored regions of the environment~\cite{Jorge2017Enabling}. A simple idea to fulfil the safety requirement is to make the boundaries of obstacles to repel the robot. Similarly, the other requirement could be met by making the boundaries (frontiers) of unexplored regions attract the robot, as shown in Figure~\ref{chp02_fig:frontier_exploration}~\cite{Thrun2006Probabilistic}.
...
The robotics research community has published several approaches for exploratory tasks, and among them we highlight the the boundary-value problem (BVP)~\cite{Prestes2002Exploration, Prestes2003BVPExploration, Prestes2011Exploration}. Initially, \citet{Prestes2002Exploration} proposed an approach for the extension of the BVP, that was first used for path-planning by ~\citet{Connolly1993Applications}, to the exploration problem. In their work, \citet{Prestes2002Exploration} argued that harmonic functions provide optimal potential maps for robot navigation. Then, they used the harmonic potential in an algorithm for exploration in a grid map. 
