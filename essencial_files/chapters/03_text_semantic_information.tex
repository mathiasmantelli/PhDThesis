...
The Mapping module considers that a ground robot is equipped with a 2D lidar sensor. The sensor measurements are used to build a 2D occupancy grid map $\bs{m}$ of the environment, like shown in Figure \ref{fig:2dmapSegmentation_a}. In simulation, the grid map $\bs{m}$ is built using the Histogramic In-Motion Mapping (HIMM) technique~\cite{Borenstein1991Histogramic}, that takes as input the lidar sensor readings. We assume there is no error in the robot motion, as we explained in Section~\ref{sec:chap2_basic_mobile_robotics}, but this assumption holds true only for the simulation experiments. When carrying out the experiments with the physical robot, we used the SLAM system called GMapping~\cite{Grisetti2007Gmapping}.
