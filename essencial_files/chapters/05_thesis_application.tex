...
Although Jaci seems a thorough solution to fight viruses espreading, there is still room for improvements. Among the changes in both Jaci's software and hardware, we highlight two of them: the need of human aid to bring Jaci to the room that must be disinfected, and lack of object-oriented mode for the disinfection process. In its current version, Jaci relies on human assistance to be placed within the room to start the disinfecting process. This limitation becomes worse when there is a sequence of rooms that must be sinitized, because when Jaci finishes the disinfection at a certain room, it has to wait to someone pick it up and move it to the next room. The longer a waiting time between to disinfection process, the longer it takes to Jaci disinfect the last room, and hence, more ineficient it becomes. The second change that would boost Jaci's performance would be an object oriented disinfection. Nowadays Jaci does not differentiate obstacles, i.e., the map labels the regions of the environment as free, obstacle or unknown. With such representation of the environment, Jaci only finishes the disinfection process when the obstacle surfaces (or borders to the free space) have received the ideal UV-C dose. However, in some cases disinfecting the whole environment is not needed, and instead, Jaci could only focus on disinfecting the objects that have been used by people. For example, in a hospital a doctor needs to use an electrocardiograph (or any other medical equippement) that has been used before by a second doctor. In this situation, Jaci could quickly disinfect only the electrocardiograph, so the first doctor could use it. By focusing on disinfecting only the object, Jaci would be more effective in providing the most appropriate working condition for the people in the environment. We believe that our contributions presented in this thesis can provide Jaci the positional semantic information necessary for these two improvements. Below we discuss how they could be addapted for Jaci's context. 
