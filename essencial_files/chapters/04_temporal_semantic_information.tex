...
\end{equation}
and $W(h, {o}_i^h)$ is defined as
\begin{equation}
W(h, h_i^{o}) = 1 - \frac{\left | h - h_i^{o} \right |}{12} 
\label{eq:hour_weight}
\end{equation}
where $h$ is the hour the robot is performing the search, and $h_i^{o}$ is the hour the object ${o}_i$ has been detected. Here is important to mention that we use the 24-hour notation. Hence, $W(h, h_i^{o})$ is equal to one when $h$ and $h_i^{o}$ are equal, and it is zero when they are 12 hours apart from each other, i.e., the largest difference in hours between two different time stamps. In our work, Eq.~\ref{eq:hour_weight} computes the weight of the object ${o}_i$, in which the closer both arguments are, the more important the object becomes for the search. For example, if the robot has detected an object two times, at 8:00 in position A and 14:00 in position B, and it is performing the search at 10:00, it will probably start searching the object in A, as 8:00 is closer to 10:00 than 14:00.
...
\begin{equation}
{c}_* = \underset{{c}\in\mathbf{H}}{\text{arg max}}\big(\Psi({c})\big)    
\label{eq:final_equation}
\end{equation}
in which the $\Psi({c})$ is defined as
\begin{equation}
