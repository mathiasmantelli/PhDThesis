...
This section details our semantic OS system and how it works. It starts with the description of the heat map module in Sections~\ref{subsec:heat_map} and ~\ref{subsec:inverting_kernel}, to explain how it is build given the gathered data on the recording mode. Finally, Section~\ref{subsec:final_formula} presents the goal computation, which explains how our OS system estimates the most promising map regions to find the target object, i.e., the warmest spots in the heat map.
...
\end{equation}
in which the $\varphi({c})$ is defined as
\begin{equation}
...
Section~\ref{subsec:datasets_simulation} discusses the dataset and simulation setup used throughout the experiments. Section~\ref{subsec:comparison_approaches} explains the other two OS approaches that we compared against our proposal, followed by Section~\ref{subsec:simulated_experiment_setup} that presents the experimental setup to which the three OS approaches have been submitted, along with their respective performances. Section~\ref{subsec:hh106_dataset} shows the performance of our semantic OS system in estimating the person's presence given the HH106 dataset.
...
\subsection{Two other object search system to be compared against ours}
...
The \textit{Static} and \textit{Static-Inv} are characterized by only one change in the mug's position throughout the day, which happened at some time between 11:00 and 15:00. Their difference is that one is the inverted version of the other. In the results from both setups, respectively Figures~\ref{fig:static_setup} and ~\ref{fig:static_inv_setup}, when the Last Seen OS system had to find the mug at noon, it was the only one that failed, and that is due to the mug's position at 23:00 in both setups. Their last mug detection in Table~\ref{tab:objects_presence_setups} happened at 23:00 in room H for the \textit{Static}, and in room A for the \textit{Stativ-Inv}. Hence, the Last Seen system wrongly guided the robot towards the opposite room in each setup. The Brute Force and our approach found the mug in both request searches for the two setups. However, there is a considerable difference in their performances for the request search at midnight in the \textit{Static}, Figure~\ref{fig:static_setup}, and for the one at noon in the \textit{Stativ-Inv}, Figure~\ref{fig:static_inv_setup}. In such requests and setups, the mug was in the room H, as shown in Table~\ref{tab:ground_truth_mugs}, and the Brute Force system makes the robot travel a longer distance until it finds the mug, as illustrated by the robot's path in Figure~\ref{fig:BF_roomH2}. In contrast, the Last Seen and ours achieved the same goal traveling a distance three times shorter, like the examples in Figures~\ref{fig:LS_roomH2} and \ref{fig:OUR_roomH2}, respectively. For the request search at noon in the \textit{Static}, and at midnight for the \textit{Static-Inv}, the three systems have a similar result because the mug is in room A, Table~\ref{tab:ground_truth_mugs}, which is the first room the Brute Force guides the robot to, as shown in Figure~\ref{fig:BF_roomA2}. Therefore, despite not reasoning over the available data, the Brute Force presents a similar result as ours.
...
It is important to compare the results from the Last Seen and our systems. Their difference is in the number of instances about the past object's position that each considers, in which the Last Seen considers only the most recent one whereas ours considers all of them. The Last Seen often fails to find the target object because relying on the newest object detection is unreliable, and hence, the robot ends up in a room that does not contain the target object, like demonstrated in Figure~\ref{fig:no_mug_room_a}. Our system shows the advantages of using all available data for more robust estimations, such as the results from the \textit{Mobile} setup pair. For the request search at noon on \textit{Mobile}, our system estimates that it is more likely to find the mug in room A. It memorizes that the mug has been detected in room A three times, at 3:00, 7:00, and 11:00, and only two times in room H, at 5:00 and 9:00, Table~\ref{tab:objects_presence_setups}. Besides the higher occurrence in room A, 11:00 is simply one hour before noon, whereas 9:00 is three, increasing A's likelihood. 
