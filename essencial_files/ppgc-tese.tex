...
%	\item [$c$] a cell in the 2D grid map $\bs{m}$.
	\item [$K(\cdot)$] circular kernel that computes the density of the free area. Its possible profiles are identified as uniform, $UK(\cdot)$, gaussian, $GK(\cdot)$, and inverted, $IK(\cdot)$.
...
%	\item [$c_K$] centre of the kernel $K(\cdot)$.
	...
	\item [$\bs{o}$] detected object, composed by its position within~\textbf{H}, its category, the hour it has been detected, and the robot's position during the detection, i.e., ${\bs{o}} = (p^{o}, c^{o}, h^{o}, r^{o})$.
